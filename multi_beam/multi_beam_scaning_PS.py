...
from util_csv import save_csv
from util_image import save_img, save_img_xyz
from util_pattern import point_2_phi_pattern, parse_pattern_dbw, phase_2_pattern, phase_2_pattern_xyz
from util_beam_scaning import set_theta_phi_loop, get_peaks, get_peak_3rd, get_peak_5th



# ============================================= 主函数 ====================================
# 相位合成法 -- 双波束 -- 核心方法
def psm_beam_2(phase1, phase2):
    ...
    # 相位转换1bit -- 1bit
    phaseBit_mix = np.where(np.logical_or(phase_mix >= 90, phase_mix <= -90), 1, 0)
    #
    return phase_mix, phaseBit_mix


# 相位合成法 -- 四波束 -- 核心方法
def psm_beam_4(phase1, phase2, phase3, phase4):
...
def main_multi_scan_2(path_pre,
        ...
        for phi1 in phi1_loops:
            for theta2 in theta2_loops:
                for phi2 in phi2_loops:
                    ...
                    # 相位合成法 -- 核心方法
                    phase_mix, phaseBit_mix = psm_beam_2(phase1, phase2)
                    # 计算 phase_mix 的方向图
    ...
    # 保存最大旁瓣的结果
    save_img(path_pre + "worst_phase_mix.jpg", worst_phase_mix)
    ...
    save_csv(worst_phase_mix, path_pre + "worst_phase_mix.csv")
                        ...
                        for phi3 in phi3_loops:
                            for theta4 in theta4_loops:
                                for phi4 in phi4_loops:
                                    ...
                                    # 相位合成法 -- 核心方法
                                    phase_mix, phaseBit_mix = psm_beam_4(phase1, phase2, phase3, phase4)
                                    # 计算 phase_mix 的方向图
                                    # pattern_mix = phase_2_pattern(phase_mix)
                                    patternBit_mix = phase_2_pattern(phaseBit_mix)
                                    patternBit_mix_xyz, x, y, z = phase_2_pattern_xyz(phaseBit_mix)
...
if __name__ == '__main__':
    ...
    # 波束扫描 -- 相位合成法: 主函数
    # main_multi_scan_2("../../files/multi-beam-scan/PS/2-(30,30,0,-5,5,15,30,30,0,175,185,15)/",
    #                   30, 30, 0, -5, -5, 15, 30, 30, 0, 175, 185, 15)
    # main_multi_scan_2("../../files/multi-beam-scan/PS/2-(20,40,10,-30,30,10,20,40,10,150,210,10)/",
    #                   20, 40, 10, -30, 30, 10, 20, 40, 10, 150, 210, 10)
    # main_multi_scan_4("../../files/multi-beam-scan/PS/4-(30,30,0,-5,5,15,30,30,0,85,95,15,30,30,0,175,185,15,30,30,0,265,275,15)/",
    ...
    main_multi_scan_4("../../files/multi-beam-scan/PS/4-(15,15,0,-20,20,10,15,15,0,70,110,10,15,15,0,160,200,10,15,15,0,250,290,10)/",
                      15, 15, 0, -20, 20, 10, 15, 15, 0, 70, 110, 10, 15, 15, 0, 160, 200, 10, 15, 15, 0, 250, 290, 10)
